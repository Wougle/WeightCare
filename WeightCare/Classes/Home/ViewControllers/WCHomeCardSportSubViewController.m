...
- (void)loadData{
    ...
    for (int i = 0; i <arry.count; i++) {
        if ([arry[i][@"date"] isEqualToString:[NSString stringWithFormat:@"2016/09/23"]]) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_sportArr.count == 0) {
        ...
        WCHomeCardSportTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"WCHomeCardSportTableViewCell"];
        if (!cell) {
            cell=[[NSBundle mainBundle]loadNibNamed:@"WCHomeCardSportTableViewCell" owner:self options:nil][0];
        }
